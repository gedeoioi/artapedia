...
<head>
    ...
    <style>
        ...
        .btn-logout:hover { border-color: #ef4444; color: #fca5a5; background: rgba(239, 68, 68, .08); }
        .btn-logout:focus-visible { outline: 3px solid rgba(239, 68, 68, .35); outline-offset: 2px; }
        html[data-theme="light"] .btn-logout { border-color: #d4d4d8; color: #3f3f46; }
        html[data-theme="light"] .btn-logout:hover { border-color: #dc2626; color: #b91c1c; background: rgba(220, 38, 38, .06); }
...
<header class="topbar sticky top-0 z-50">
    <div class="max-w-6xl mx-auto px-4 py-3 flex items-center gap-2 sm:gap-3">
        ...
        <nav class="hidden lg:flex items-center gap-7 ml-3">
            ...
            {{-- Menu "Member" dihapus: tombol "Member Area" di kanan sudah menuju
                 halaman yang sama, jadi keduanya hanya mengulang tujuan. --}}
        </nav>
        <div class="ml-auto flex items-center gap-2">
            ...
            @auth
                ...
                {{-- Gaya sekunder supaya tidak bersaing dengan tombol utama di sebelahnya. --}}
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="btn-logout px-3 sm:px-4 py-2 text-sm whitespace-nowrap" aria-label="Keluar dari akun">Keluar</button>
                </form>
            @else
